...
class Jugador(Thread):
    ...
    def perder_bandera(self) -> None:
        if self.lock_bandera.acquire(blocking=False):
            if self.tiene_bandera:
                self.tiene_bandera = False
                self.bandera.actualizar_dueño(None)
    ...
    def capturar_bandera(self) -> None:
        "captura la bandera"
        if self.lock_bandera.acquire(blocking=False):
            for jugad in self.rivales:
                jugad.perder_bandera()
                jugad.tiene_bandera = False
    ...
    def correr_segunda_mitad(self) -> bool:
        with self.lock_carrera:
            if self.senal_fin.is_set(): #REVISAR EL LEVANTAMIENTO
                self._correr = False
                return False
            elif self.posicion >= 100 and self.tiene_bandera:
                print('Se acabo la carrera')
                self._correr = False
                self.senal_fin.set()
                return True
            else:
    ...
    def run(self) -> None:
        # Completar solo el paso 1. de este método
